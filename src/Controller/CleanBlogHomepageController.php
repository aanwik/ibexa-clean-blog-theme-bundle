...
class CleanBlogHomepageController extends AbstractController
{
    ...
    public function showHomepageAction(ContentView $view): ContentView
    {
        return $this->repository->sudo(function () use ($view) {
            $location = $view->getLocation();

            // Fetch all blog posts globally
            ...
            foreach ($searchResult->searchHits as $hit) {
                $blogPosts[] = $this->contentService->loadContentByContentInfo($hit->valueObject->contentInfo);
            }

            $view->addParameters([
                'ibexa_location' => $location,
                'ibexa_content' => $view->getContent(),
